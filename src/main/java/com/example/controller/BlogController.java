...
public class BlogController {
    ...
    public Result<?> findPage(@RequestParam(required = false, defaultValue = "") String name,
        ...
        query.eq(Blog::getAudit, "是");
        return Result.success(blogService.page(new Page<>(pageNum, pageSize), query));
    }

    @GetMapping("/getPage")
    public Result<?> getPage(@RequestParam(required = false, defaultValue = "") String name,
                              @RequestParam(required = false, defaultValue = "") String tag,
                              @RequestParam(required = false, defaultValue = "1") Integer pageNum,
                              @RequestParam(required = false, defaultValue = "10") Integer pageSize) {
        LambdaQueryWrapper<Blog> query = Wrappers.<Blog>lambdaQuery().orderByDesc(Blog::getId);
    ...
    public List<Blog> getlist(){
        ...
        List<Blog> collect = list.stream().limit(8).collect(Collectors.toList());
        return collect;
    }
